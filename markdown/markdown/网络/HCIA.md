...
![一台空路由器的ip路由表](../../img/网络/HCIA/一台空路由器的ip路由表.png)
### 路由信息的获取方式
...
步骤 1 终端设备不停的检测共享线路的状态。
步骤 2 如果线路空闲则发送数据。
步骤 3 如果线路不空闲则一直等待。
步骤 4 如果有另外一个设备同时发送数据，两个设备发送的数据必然产生冲突，导致线路上的信号不稳定。
步骤 5 终端设备检测到这种不稳定之后，马上停止发送自己的数据。
步骤 6 终端设备发送一连串干扰脉冲，然后等待一段时间之后再进行发送数据。发送干扰脉冲的目的是为了通知其他设备，特别是跟自己在同一个时刻发送数据的设备，线路上已经产生了冲突。
步骤 7 CSMA/CD 的工作原理可简单总结为：先听后发，边发边听，冲突停发，随机延迟后重发
### 广播域
